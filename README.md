...
- 표준 `simulation_interfaces`를 통한 host simulation 상태·world 제어
- 현재 Ubuntu x86_64 host에서 검증하는 amd64 core 이미지
...
이 명령은 core-only 개발용이며 host-network DDS overlay를 적용하지 않습니다.
Host bridge 실습에서는 `./run.sh dev`가 아니라 `./run.sh isaac-host-dev`를 사용합니다.

### 5. Host Isaac Sim 실행

먼저 ROS2 container를 host DDS profile로 시작합니다.
...
않으며, `--full` 실행에는 충분한 로컬 디스크가 필요합니다. CI의 실행 지원 계약은
amd64 core 빌드와 smoke test입니다. Lock과 이미지 인덱스의 architecture 검사는
데이터 수준의 이식성 guard이며 ARM64 runtime 지원을 의미하지 않습니다.
