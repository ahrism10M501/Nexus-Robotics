...
└── bb0d49742fe96eba0a9492d770c92809a8b6a6ff  approved design
    └── 744a8a9bda98dd6b7fd50a0703bf6fefab981bc5  user/damin, approved plan
        └── e6da3b444d501e2175517efb4c5b983b5fa5701b  refactor/core-branch-layout,
            ...
            └── aa26a0274dae6baa7b0626bf0cf883eff035d088  first contract correction
                └── current forward execution-gap correction
...
계열을 x86_64 marker로 제한한다. 여섯 direct pin은 `.in`과 lock에 각각 정확히
한 번만 있어야 한다. generation과 `--validate-only`는 QEMU 없이 동일한 native
pinned uv/ROS 환경에서 실행한다. validation은 `--python-version 3.12`와
`--python-platform x86_64-manylinux_2_39`, 그리고
`--python-platform aarch64-manylinux_2_39`를 각각 사용한 wheel-only,
`--require-hashes` dry-run이다. image index test가 두 pin의 amd64/arm64 child를
증명한다. 실제 arm64 실행은 authoritative CI와 native/QEMU preflight를 통과한
로컬 final acceptance에서만 검증한다. image 설치에서는
`uv pip sync --require-hashes`와 `uv pip check`를 실행하고 direct import, exact
Python 3.12, exact uv 0.8.3을 smoke test한다.

repository command script는 직접 argv로 실행하므로 generator, `run.sh`, doctor,
launcher, Isaac acceptance, check wrapper는 0755를 contract test한다. 개별 test file은
`bash tests/...`로 실행하며 executable mode를 요구하지 않는다.
...
`DEVELOPER_GID`를 Docker `build.args`로 전달한다. direct Dev Container startup을 위해
Compose의 ROS/uv default는 `docker/versions.env`의 exact literal을 복제하되 static test로
두 값의 일치를 강제한다. ID default는 `${LOCAL_UID:-1000}`과
`${LOCAL_GID:-1000}`, runtime `user`는 항상 `developer`이다. `run.sh`는 `.env`의
양의 ID를 data로 parsing하여 build arg에 전달하고 Dockerfile이 0을 거부한다.

normalized Compose test는 두 service의 `user: developer`와 build ID 일치를 확인한다.
기본에서 `privileged`, `cap_add`, `devices`, device cgroup rule, GPU, GUI, host
network, PID, IPC, Docker socket, fixed name을 거부/검사한다. AI service는 host/GPU/GUI
3개 override의 전체 2^3 power set을, core는 base/host/GUI 지원 조합을 모두
render한다. workspace를 포함한 모든 bind는 long syntax의 `type: bind`와
`bind.create_host_path: false`를 사용하고 Xauthority/X11은 `read_only: true`이다.
로컬은 Compose `>=2.30.0`을 semantic version으로 검사하고 CI만 exact 2.30.3을
강제한다.
...
거부한다. profile 파일 자체와 Compose/check 경로를 canonicalize하여 repo
안에 있음을 확인하고 safe `SERVICE` identifier를 강제한다. command path의
syntax/containment는 load 시점에 검증하지만 존재/executable 검사는 doctor/check
액션 dispatch 시점까지 미룬다.

`init`, `doctor`, `check`는 Compose 선행 검증을 하지 않는다. 그래야 doctor가
Docker 누락을 진단할 수 있다. `build`, `up`, `shell`, `dev`, `status`, `down`
액션만 normalized Compose service 존재를 먼저 확인한다. standard doctor는
`./run.sh doctor`와 `./run.sh isaac-host-doctor`이며 `<profile>-doctor`를 generic하게
해석한다. 두 profile의 `CHECK_COMMAND`는 executable
`scripts/check_dev_workflow.sh`이다.
...
경로를 모두 portable 값으로 바꾴다. retained
`docs/tutorials/shared/troubleshooting.md`도 명시적 audit 대상이다. governance spec/plan을 history 자료로
제외한 나머지 README/docs에는 Doosan/A0912/OpenArm runtime 경로, command,
target과 service가 남지 않아야 한다. 삭제 대상의 완전한 path inventory를
계획과 permanent static test에 명시하고 모든 path가 preservation tag에 있음을
각각 검증한다. 31개 전체는 staging 전 filesystem에서 없어야 하고 exact staging 후
Git index에서도 없어야 한다.
...
보존 검사는 외부 `root-worktree.manifest`
(`ebd801615fe1d523757048ba1f6f884f57956af7181a74f99cc3e7fc3a0e3780`)와
`feature-worktree.manifest`
(`09a318b77f6447e78ca9801db3f4ba7583f54c5dd0edaeaa5ea07fd3190eeb5e`)를
사용한다. root HEAD `744a8a9bda98dd6b7fd50a0703bf6fefab981bc5`, feature HEAD
`bbce9bdb91a76ed57755542586bfcd6e0af61ba9`, NUL-delimited status/untracked-list hash,
full binary diff hash, manifest에 나열된 모든 tracked/untracked 파일 hash를 이관 전과
integration 직전에 재검증한다. `user-ws/damin/tutorial-7.py`도 명시적
보존 대상이다.
...
5. 두 image index의 amd64/arm64 manifest 검증과 pinned QEMU/Buildx CI substrate
...
8. 로컬 Compose `>=2.30.0` 및 CI exact 2.30.3에서 base/override 전체 render

CI는 image를 push하지 않고 vendor target, Isaac install, GPU/하드웨어 명령을
실행하지 않는다.

authoritative multi-arch CI는 `runs-on: ubuntu-24.04`에서 full commit으로 pin된
checkout/QEMU/Buildx/Compose action을 사용한다. controller가 확인한 substrate는
`tonistiigi/binfmt@sha256:400a4873b838d1b89194d982c45e5fb3cda4593fbfd7e08a02e76b03b21166f0`,
Buildx `v0.35.0`,
`moby/buildkit@sha256:0168606be2315b7c807a03b3d8aa79beefdb31c98740cebdffdfeebf31190c9f`,
Compose `v2.30.3`이다. 로컬 acceptance는 native arm64 또는 이미 설정된 QEMU/binfmt
실행을 먼저 probe한다. 둘 다 없으면 integration을 `HOLD`하고 privileged
binfmt 등록에 대한 명시적 권한을 요청하며 자동 등록하지 않는다. 현재
host는 `qemu-aarch64`가 없으므로 이 gate가 예상된다.
...
Isaac installation 또는 host 선행 조건이 없을 때만 automated host acceptance를
exit 77 `SKIP`으로 기록한다. 호환 installation, NVIDIA, launcher, version 조건이
있는데 graph/topic discovery 또는 `/clock` 관측이 안 되면 non-77 `FAIL`이다.
