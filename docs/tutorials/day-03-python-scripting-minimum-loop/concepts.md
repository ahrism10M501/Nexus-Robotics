...
"방금 손으로 조금 다르게 놓은 cube"가 문제가 됩니다. 후속 deterministic task에서는
...
그렇다면 대개 `setup_scene`의 책임입니다. 예를 들어 후속 실습의 `/World/Cube`는
episode마다 필요한 task object이므로 code에서 명시적으로 만들거나 reference해야 합니다.
...
진행될 때 내 code도 같이 호출되는 자리"라는 점을 확인합니다. 이 감각은 후속 실습에서
observation과 action을 episode 단위로 저장할 때 다시 쓰입니다.
...
후속 deterministic task에서는 reset이 특히 중요합니다. cube 위치가 매번 다르면 pick
