...
Doosan container를 시작하고 shell로 들어갑니다. 처음이거나 Dockerfile이 바뀐 뒤에는
build를 먼저 실행합니다.
...
Doosan container 안에서 기본 ROS2 환경을 source합니다.
...
이 repo의 `doosan_dev` image에는 Doosan Jazzy workspace가 build되어 있고,
`/etc/profile.d/nexus_env.bash`가 자동으로 source합니다. launch command를 실행하기 전에
package가 보이는지 확인합니다.
...
Doosan virtual mode는 emulator를 위해 host Docker daemon을 사용할 수 있어야 합니다.
아래 command가 실패하면 host에서 `./run.sh doosan-dev`로 Doosan container에 들어왔는지
확인합니다. container만 따로 띄울 때는 `./run.sh doosan-up`을 사용합니다.

```bash
docker ps
```

emulator image가 아직 설치되지 않았다면 한 번 준비합니다. 이 image는 host Docker에
남기 때문에 `doosan_dev` container를 껐다 켜는 것만으로는 다시 실행할 필요가 없습니다.
Docker image를 지웠거나 emulator version을 바꿨을 때만 다시 실행합니다.
...
이 command가 실패하면 일반 `ros2_dev` shell에 들어온 것입니다. host에서
`./run.sh doosan-dev`로 Doosan container에 들어가세요. 이미 Doosan container가
떠 있고 추가 shell만 필요하면 `./run.sh doosan-shell`을 사용합니다.

Doosan virtual emulator가 시작되지 않으면 `docker ps`가 `doosan_dev` 안에서 성공하는지
확인하고, 필요하면 `bootstrap_doosan_emulator`를 다시 실행합니다.
